...
if($q == false)
{
    echo "<span class='search_page'>
            <p class='search_not'>По вашему запросу ничего не найдено.</p>
            <img src='/img/search_not.png'>
        </span>";
} 
else 
{
...
?>
    <div id='tagsDiv'>
        <p class='sum_video'>Найдено <?=$sum_video;?> видео</p>
<?php
    foreach ($q as $value) {
...
?>
    <br><div id='trendDiv'>
    <form action='/'><!--Перенаправляет на главную страницу-->
        <button class='button_trend' type='submit' name=''>В тренде</button>
    </form>
    </div>
<!--Эта кнопка понадобиться еще в других местах-->